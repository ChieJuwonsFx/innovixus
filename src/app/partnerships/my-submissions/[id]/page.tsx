...
const CancelModal = ({
  ...
  return (
    ...
    >
      <div
        className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xl max-w-md w-full p-6"
      ...
      >
        ...
        <p className="text-sm text-slate-600 dark:text-slate-300 mb-6">
          Apakah Anda yakin ingin membatalkan pengajuan partnership ini? Status
          pengajuan dan event akan diubah menjadi Dibatalkan.
        </p>
...
const Badge = ({
  ...
  <span
    className={clsx("px-2.5 py-1 text-xs font-semibold rounded-full", {
...
const StatusBadge = ({
  ...
  const getPaymentBadge = (status: string) => {
    switch (status) {
      case "Paid":
        return (
          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-semibold rounded-full w-full justify-center bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-300">
      ...
      case "Pending":
        return (
          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-semibold rounded-full w-full justify-center bg-yellow-100 text-yellow-800 dark:bg-yellow-900/50 dark:text-yellow-300">
            <Clock size={14} />
            Verifikasi
          </span>
        );
      ...
      case "Canceled":
        return (
          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-semibold rounded-full w-full justify-center bg-red-100 text-red-800 dark:bg-red-700 dark:text-red-300">
            <XCircle size={14} />
            Batal
          </span>
        );
  ...
  const getEventBadge = (status: string) => {
    switch (status) {
      ...
      case "Canceled":
        return (
          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-semibold rounded-full w-full justify-center bg-red-100 text-red-800 dark:bg-red-900/50 dark:text-red-300">
            <XCircle size={14} />
            Ditolak
          </span>
        );
...
const getPosterUrl = (posterData: Json): string | null => {
  if (
    Array.isArray(posterData) &&
    posterData.length > 0 &&
    posterData[0] &&
    typeof posterData[0] === "object" &&
    "url" in posterData[0] &&
    typeof posterData[0].url === "string"
  ) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const { data, error: fetchError } = await supabase
        .from("partnerships")
        .select(
          `*, packages ( * ), events!partnerships_event_id_fkey ( *, organizers ( * ) )`
        )
        .eq("id", id)
        .single();
      ...
      <div className="min-h-screen">
        <div className="container mx-auto px-4 py-20 sm:py-24">
          ...
          <div className="mb-8">
            ...
            <p className="mt-1 text-slate-500 dark:text-slate-400 font-mono text-xs">
              ID: {submission.id}
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            <div className="lg:col-span-2 bg-white dark:bg-slate-900 p-6 sm:p-8 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
              <div className="space-y-8">
                {events && (
                  <div>
                    <h3 className="text-lg font-semibold border-b pb-2 mb-2 border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-200">
                      Detail Event
                    </h3>
                    <dl className="divide-y divide-slate-200 dark:divide-slate-700">
                      <InfoRow icon={<Ticket size={16} />} label="Judul Event">
                        {events.title}
                      </InfoRow>
                      <InfoRow icon={<Tag size={16} />} label="Kategori">
                      ...
                      <InfoRow icon={<Globe size={16} />} label="Platform">
                        <Badge text={events.is_online} color="blue" />
                      </InfoRow>
                      <InfoRow icon={<MapPin size={16} />} label="Lokasi">
                        {events.location || "-"}
                      </InfoRow>
                      <InfoRow
                      ...
                      >
                        {formatDate(events.open_date)}
                      </InfoRow>
                      <InfoRow
                        icon={<Clock size={16} />}
                        label="Tutup Pendaftaran"
                      >
                        {formatDate(events.close_date)}
                      </InfoRow>
                      <InfoRow
                ...
                {events?.organizers && (
                  <div>
                    ...
                    <dl className="divide-y divide-slate-200 dark:divide-slate-700">
                      ...
                      >
                        {events.organizers.name}
                      </InfoRow>
                      <InfoRow icon={<Instagram size={16} />} label="Instagram">
                        @{events.organizers.instagram}
                      </InfoRow>
                    </dl>
                  </div>
                )}
